...
export class TinkUserService {
  ...
  public async createNewUser(input: CreateUserInput): Promise<CreateUserObject> {
    return this.tinkHttpService.post<CreateUserObject, CreateUserInput>(
      '/api/v1/user/create',
      input,
    );
  }
  ...
  public async delegateAuthorizationToUser(input: CreateDelegatedAuthorizationInput): Promise<string> {
    const authorization: CreateAuthorizationObject = await this.tinkHttpService
      .post<CreateAuthorizationObject, CreateDelegatedAuthorizationInput>(
