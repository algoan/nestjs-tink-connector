...
export class TinkProviderService {
  constructor(@Inject(CONFIG) private readonly config: Config, private readonly tinkHttpService: TinkHttpService) {}
  ...
  public async getProviders(): Promise<TinkProviderObject[]> {
    ...
    const response: TinkProviderListResponseObject<null> = await this.tinkHttpService.get<
      TinkProviderListResponseObject<null>,
      TinkProviderListArgs
    >('/api/v1/providers', args);
